...
#' @param ShinySoSV_DIR directory of installed ShinySoSV
#' @return data frame of predicted performance
#' @export
ShinySoSV_prediction <- function(Candidate_callers, newdata, performance, callset, ShinySoSV_DIR){
  ...
  if(!file.exists(paste0(ShinySoSV_DIR,"/data/","gam",model_name[1],"_callers.RData"))){
    cat("Please provide the directory of ShinySoSV.\n")
  }else{
    for(i in c(1:length(model_name))){
      load(paste0(ShinySoSV_DIR,"/data/","gam",model_name[i],"_callers.RData"))
    }
